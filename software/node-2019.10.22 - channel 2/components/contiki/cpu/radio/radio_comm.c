...
{
  static SEGMENT_VARIABLE(cmd_buf[256], U8, SEG_DATA);
  SEGMENT_VARIABLE(cmd = 0x44, U8, SEG_DATA);
  ...
  {
    radio_hal_ClearNsel();
    radio_hal_SpiWriteThenRead(1, &cmd, byteCount + 1, cmd_buf);//read CMD buffer
    ctsVal = cmd_buf[0];
    if (ctsVal == 0xFF)
    {
      if (byteCount)
      {
        memcpy(pData, &cmd_buf[1], byteCount);
      }
      radio_hal_SetNsel();
      break;
    }
...
void radio_comm_ReadData(U8 cmd, BIT pollCts, U8 byteCount, U8* pData)
{
    ...
    radio_hal_ClearNsel();
    radio_hal_SpiWriteThenRead(1, &cmd, byteCount, pData);
    radio_hal_SetNsel();
    ctsWentHigh = 0;
}
...
void radio_comm_WriteData(U8 cmd, BIT pollCts, U8 byteCount, U8* pData)
{
    ...
    radio_hal_ClearNsel();
    radio_hal_SpiWriteThenWrite(1, &cmd, byteCount, pData);
    radio_hal_SetNsel();
    ctsWentHigh = 0;
}
...
U8 radio_comm_SendCmdGetResp(U8 cmdByteCount, U8* pCmdData, U8 respByteCount, U8* pRespData) {
    ...
    /* make sure this operation could not be interrupt */
    rt_enter_critical();

    radio_comm_SendCmd(cmdByteCount, pCmdData);
    result = radio_comm_GetResp(respByteCount, pRespData);
